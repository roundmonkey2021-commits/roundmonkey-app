...
interface AvgWinnersVsLosersDistributionChartProps {
  trades: Trade[];
  isDark: boolean;
  onClose?: () => void;
}

export function AvgWinnersVsLosersDistributionChart({ trades, isDark, onClose }: AvgWinnersVsLosersDistributionChartProps) {
      ...
      <Card className={isDark ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-neutral-200'}>
        <CardHeader>
          <CardTitle className={isDark ? 'text-zinc-100' : 'text-neutral-900'}>
            Avg Winners vs Losers Distribution
          </CardTitle>
        </CardHeader>
        <CardContent>
  ...
  const getChartTitle = () => {
    if (timeFrame === 'days') return 'Daily Avg Winners vs Losers Distribution';
    if (timeFrame === 'weeks') return 'Weekly Avg Winners vs Losers Distribution';
    return 'Monthly Avg Winners vs Losers Distribution';
  };

  const getChartDescription = () => {
    if (timeFrame === 'days') return 'Green bars show average profit per winning trade, red bars show average loss per losing trade per day';
    if (timeFrame === 'weeks') return 'Green bars show average profit per winning trade, red bars show average loss per losing trade per week';
    return 'Green bars show average profit per winning trade, red bars show average loss per losing trade per month';
  };
