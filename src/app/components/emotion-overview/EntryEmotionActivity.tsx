...
interface EntryEmotionActivityProps {
  trades: Trade[];
...
export function EntryEmotionActivity({
  trades,
  granularity,
  range,
  selectedPhases,
  ...
  const chartData = useMemo(() => {
    ...
    // Extract unique entry emotions from actual trade data
    ...
    // Group trades by period (based on exit date)
    ...
    filteredTrades.forEach(trade => {
      ...
      if (isNaN(date.getTime())) {
        console.warn('Invalid exit date for trade:', trade.exitDate);
        return;
      }
    ...
    // Build data points with emotion counts
    const dataPoints = [];
    for (const [periodKey, periodTrades] of periodMap.entries()) {
      ...
      if (granularity === 'daily') {
        ...
        if (isNaN(d.getTime())) {
          console.warn('Invalid period key for daily:', periodKey);
          continue;
        }
        ...
        const d = new Date(periodKey);
        if (isNaN(d.getTime())) {
          console.warn('Invalid period key for weekly:', periodKey);
          continue;
        }
        ...
        if (isNaN(d.getTime())) {
          console.warn('Invalid period key for monthly:', periodKey);
          continue;
        }
      ...
      // Count occurrences of each emotion in this period
      displayEmotions.forEach(emotion => {
        point[emotion] = periodTrades.filter((t: Trade) => t.entryEmotions === emotion).length;
      });
  ...
  // Initialize selected emotions with all emotions on first load only
  useEffect(() => {
        ...
        <div className="flex items-start justify-between gap-4 mb-2">
          <div>
            <h2 className={`text-lg font-semibold ${isDark ? 'text-zinc-200' : 'text-neutral-900'}`}>
              Entry Emotion Activity — {granularityLabel}
            </h2>
            <p className={`text-xs mt-1 ${isDark ? 'text-zinc-400' : 'text-neutral-500'}`}>
              Number of trades over time grouped by Entry emotion
            </p>
          </div>
        ...
        <div className="flex items-center justify-center h-[350px]">
          <div className="text-center">
            ...
            <p className={`text-xs mt-1 ${isDark ? 'text-zinc-500' : 'text-neutral-400'}`}>
              Adjust your filters or add entry emotions to trades
            </p>
          </div>
        </div>
      )}
